...
window.onload = function () {
  // 页面加载完成后初始化事件
  wf = new _waterfall__WEBPACK_IMPORTED_MODULE_1__["default"]('.container', 4, 'items', 5);
}; // 监听滚动
      ...
      if (container) {
        ...
        if (children && children.length > 0) {
          ...
          for (var i = 0; i < children.length; i += 1) {
            ...
            if (i >= this.$num) {
              elTop = Number(children[i - this.$num].style.top.replace('px', '')) + Number(children[i - this.$num].offsetHeight);
            } // 设置样式


            children[i].style.cssText = 'width:' + this.$width + '%; position: absolute; left: ' + elLeft * this.$width + '%; top: ' + elTop + 'px;box-sizing: border-box;padding: ' + this.$padding + 'px';
          }
